...
- `Stx3.yaml`: **revertido meu erro** — eu havia trocado `total: 32` por `64`; o datasheet (STX3 User Manual 8545-0198-01 R-4) diz **32 pinos** (Table 3 vai de 1 a 32). O módulo é de 4 lados, mas **9/7/9/7**, não 16 por lado — o campo errado era o `por_lado`, não o `total`. E o bloco `numeracao: {inicio_esquerdo: 1, inicio_direito: 17}` que removi **estava correto** (com 9 à esquerda, o direito começa mesmo em 17). Agora usa `lados: {esquerdo: 9, base: 7, direito: 9, topo: 7}` e `pitch: 2.54` / pad `2.03 × 1.93` (as cotas reais: `.100"` e `.076 × .080"`; os valores anteriores — pitch 1,778 e pad 2,03×1,27 — eram inventados; o próprio comentário admitia ter escolhido o pitch "para caber 16 pinos"). **O arquivo segue marcado como NÃO USÁVEL**: a fileira inferior tem pitch não-uniforme (`.100 .100 .100 .120 .120 .100`, folga extra ao redor do pino 14 = RFOUT) que o `quad_smd` não expressa — a peça exige `padrao: custom` —, e os datums dos offsets da Fig.7 não são determináveis da figura raster. Destravar depende do Reference Design Package (Ref [A], Gerbers, sob pedido à Globalstar).
